                    ...
                    <Table.Body>
                        {#each positions as p}
                            <Table.Row>
                                ...
                                <Table.Cell class={(profit.comparedTo(0) > 0 ? "text-[#ef5350]" : "text-[#26a69a]") + " font-bold"}>
                                    <span>{market?.parse_quote(profit)}</span><span class="text-[10px] pl-1">{profit.div(new Decimal(p.initialMargin)).mul(new Decimal("100")).toFixed(2)}%</span>
                                </Table.Cell>
