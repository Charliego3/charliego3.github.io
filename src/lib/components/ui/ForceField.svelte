  ...
  outColor = vec4(col, alpha);
    ...
    export function createForceField(
        ...
        function syncCanvasSize() {
            ...
            for (let i = MIPS - 1; i >= 0; i--) {
                const up =
                    i === 0
                        ? {
                              width: brightTarget.width,
                              height: brightTarget.height,
                          }
                        : downTargets[i - 1];
        ...
        function render() {
            ...
            gl!.uniform1f(
                fu.uGridRevealMode,
                REVEAL_INDEX[config.gridReveal] ?? 2,
            );
            gl!.uniform1f(
            ...
            gl!.uniform1f(
                fu.uFlashIntensity,
                clamp(config.flashIntensity, 0, 1),
            );
            ...
            gl!.uniform1f(
                fu.uEdgeFalloff,
                clamp(config.edgeFalloff, 0.02, 0.6),
            );
            gl!.uniform1f(fu.uReveal, clamp(config.reveal, 0, 1));
            gl!.uniform1f(
                fu.uDissolveScale,
                clamp(config.dissolveScale, 0.5, 12),
            );
            gl!.uniform1f(
                fu.uDissolveWidth,
                clamp(config.dissolveWidth, 0.005, 0.2),
            );
            ...
            gl!.uniform1f(
                fu.uRippleWidth,
                clamp(config.rippleWidth, 0.01, 0.4),
            );
            gl!.uniform1f(fu.uRippleBlend, clamp(config.rippleBlend, 0, 1));
            gl!.uniform1f(
                fu.uRippleDuration,
                clamp(config.rippleDuration, 0.3, 5),
            );
            gl!.uniform1f(
                fu.uRippleIntensity,
                clamp(config.rippleIntensity, 0, 8),
            );
            gl!.uniform1f(
                fu.uRippleMaxRadius,
                clamp(config.rippleMaxRadius, 0.1, 2),
            );
            ...
            gl!.uniform1f(
                fu.uHoverRadius,
                clamp(config.hoverRadius, 40, 600) * dpr,
            );
            ...
            if (bloomOn) {
                ...
                for (let i = 0; i < MIPS; i++) {
                    gl!.bindFramebuffer(
                        gl!.FRAMEBUFFER,
                        downTargets[i].framebuffer,
                    );
                    gl!.viewport(
                        0,
                        0,
                        downTargets[i].width,
                        downTargets[i].height,
                    );
            ...
            bindTargetTexture(
                1,
                bloomOn ? upTargets[0].texture : sceneTarget.texture,
            );
            ...
            gl!.uniform1f(
                compositePass.uniforms.uGrain,
                clamp(config.grain, 0, 1),
            );
        ...
        function spawnHit(x: number, y: number) {
            ...
            hitPos[idx * 2] = x * dpr;
            hitPos[idx * 2 + 1] =
                output.height - y * dpr - content.scrollTop * dpr;
            hitTime[idx] = time;
...
<script lang="ts">
    ...
    interface Props extends ForceFieldOptions {
        class?: string;
