    ...
    export function createCloth(
        ...
        function syncCanvasSize() {
            ...
            contentMaxX = Math.min(
                ...
                Math.max(
                    0.05,
                    content.clientWidth / Math.max(wrapper.clientWidth, 1),
                ),
            );
            if (htmlInCanvas) {
        ...
        function stepSim(dt: number) {
            ...
            for (let a = 0; a < NODES; a++) {
                colForce[a] =
                    (0.7 + 0.3 * Math.sin(ka * a - 1.7 * t)) * hangCurve[a];
            }

            const c2 = WAVE_SPEED * WAVE_SPEED;
            const dt2 = dt * dt;
            const decay = Math.exp(
                -Math.min(Math.max(config.damping, 0.05), 8) * dt,
            );
            for (let y = 0; y < NODES; y++) {
        ...
        function composeVertices(width: number, height: number) {
            ...
            for (let y = 0; y < NODES; y++) {
                ...
                for (let x = 0; x < NODES; x++) {
                    ...
                    const dzdy =
                        (zField[down + x] - zField[up + x]) / (2 * cellH);
            ...
            if (probeCtx) {
                ...
                while (el) {
                    ...
                    if (bg && bg !== "transparent") {
                        ...
                        const [r, g, b, a] = probeCtx.getImageData(
                            0,
                            0,
                            1,
                            1,
        ...
        function render() {
            ...
            gl!.uniform1f(
                cloth.uniforms.uFocal,
                Math.max(config.perspective, 200),
            );
        ...
        function frame(now: number) {
            ...
            if (!reducedMotion) {
                ...
                const omega = 14;
                touch.vx +=
                    ((pointer.x - touch.x) * omega * omega -
                        2 * omega * touch.vx) *
                    delta;
                touch.vy +=
                    ((pointer.y - touch.y) * omega * omega -
                        2 * omega * touch.vy) *
                    delta;
            ...
            setOptions(next) {
                if (
                    !Object.entries(next).some(
                        ([key, value]) =>
                            config[key as keyof ClothOptions] !== value,
                    )
                )
                    return;
