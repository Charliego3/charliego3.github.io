  ...
  outColor = vec4(col, alpha);
    ...
    export function createBlaze(
        ...
        function ensureFireTarget() {
            ...
            if (fireTexture && width === fireWidth && height === fireHeight)
                return;
            fireWidth = width;
            fireHeight = height;
            ...
            setOptions(next) {
                if (
                    !Object.entries(next).some(
                        ([key, value]) =>
                            config[key as keyof BlazeOptions] !== value,
                    )
                )
                    return;
...
<script lang="ts">
    ...
    interface Props extends BlazeOptions {
        class?: string;
